...
def _should_answer(scores):
    # Case 1: Very strong single evidence (atomic fact)
    if scores[0][1] >= HIGH_CONF_THRESHOLD:
        return True

    # Case 2: Multiple supporting evidence
    if (
        scores[0][1] >= PRIMARY_THRESHOLD
        and scores[1][1] >= SECONDARY_THRESHOLD
    ):
        return True
